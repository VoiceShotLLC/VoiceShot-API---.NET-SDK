   ...
   <script language="C#" runat=server>
	   ...
	   Boolean ParseXML(string XMLContent)
	   {
		 try
		 {
		   ...
		   String MenuID, Duration, CallerID, CallID, DateAndTime, VoiceFileName, PhoneNumber, Status, Comment;
		   XmlNode TempNode;
		   Byte[] VoiceFile;
		   ...
		   DateAndTime = attrColl["dateandtime"].Value;
		   CallID      = attrColl["callid"].Value;
		   PhoneNumber = attrColl["phonenumber"].Value;
		   Status      = attrColl["status"].Value;
		   Comment     = attrColl["comment"].Value;

		   //writed parsed values to file
		   StreamWriter w = File.AppendText(Request.MapPath("summaryoutcall.txt"));
		   ...
		   w.WriteLine("Phone Number: " + PhoneNumber);
		   w.WriteLine("Status: " + Status);
		   if (Comment != "") {
		    w.WriteLine("Call Information: " + Comment);
		   }
	   ...
	   void Page_Load(object sender, System.EventArgs e)
	   {
		  try
		  {
			 ...
			 StreamWriter w = File.AppendText(Request.MapPath("xmlsummaryoutcall.txt"));
			 w.WriteLine("--- "  + DateTime.Now + " ------------------------------------------------------");
